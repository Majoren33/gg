...
local unclaimed = {}
local counter = 0             
local mainCheckPosition = workspace.Map.Festive.ChristmasTrees.MainChristmasTree.Trunk.Position * Vector3.new(1,0,1)
...
getgenv().settings = {
	...
	render = true,
	boothSwitcher = false,
	customBoothText = `💰 GOAL $G\nPlease Kindly Help So I Can Not Be A Bacon 😢\nI Get Treated Badly As A Bacon 😭`,
	serverHopDelay = 15,
	antiBotServers = true,
	...
	donateResponce = {"sorry im saving", "i am saving for my dream item", "sorry my robux is pending"},
    ...
    begMessage = {"1 Robux = 1 JUMP", "Jumping for donations!", "Please i get bullied for being a bacon it would help a lot so i can change my outfit", "Please Kindly Help So I Can Not Be A Bacon I Get Treated Badly As A Bacon 😭"},
	fpsBoost = false,
	...
	webhookToggle = true,
	gravitySwitch = false,
	webhookBox = "https://discord.com/api/webhooks/1314001308637728779/5wbBA7oybEe_tSKPWWJPTMtQGwHk4US5lpcsC_fjwpAnq4es98RVORhCvHJ9aOV2923a",
...
local function BoostFPS()
    ...
    camera.CFrame = CFrame.new(0, 1000, 0) -- Position the camera far away
	if not getgenv().settings.ClearMap then
		getgenv().settings.ClearMap = true
		local Folder = Instance.new("Folder", workspace)
		Folder.Name = "Protected"

		local Map = workspace.Map

		Map.Environment.Path.Parent = Folder
		Map.Environment.Terrain.Parent = Folder
		Map.Functional.SpawnLocations.Parent = Folder

		workspace.BoothModels.Parent = Folder
		workspace.BoothInteractions.Parent = Folder


		for i, v in next, workspace:GetDescendants() do
			if v:IsA("Terrain") or v:IsA("Camera") or v.Name == "MapUI"  or v.Name == "SoundRegions" or v == Folder or v:IsDescendantOf(workspace.SoundRegions) or v:IsDescendantOf(game.Players.LocalPlayer.Character) or v:IsDescendantOf(Folder) then
				continue
			end

			if  v~= Map and v ~= game.Players.LocalPlayer.Character then
				v:Destroy()
			elseif v == Map then
				for i, x in next, Map:GetChildren() do
					x:Destroy()
				end
			end
		...
		Folder.Path.Stones:Destroy()

		for i, v in next, Folder.Terrain:GetChildren() do
		...
		Folder.BoothModels.Parent = workspace
		Folder.BoothInteractions.Parent = workspace
		
		workspace.ChildAdded:Connect(function(Child)
			task.wait()
			if Child.Name == "RobuxNuke" or Child.Name == "RobuxHammerGiant" then
				Child:Destroy()
			end
		end)
...
local function webhook(raised, donor)
	...
	local a = {
		...
		["footer"] = {
			["text"] = "made by @sszze / https://discord.gg/e5TgSFnrq",
		},
...
highlightTab:AddLabel('(or join manually if your executor sucks: discord.gg/SaGSTVmKM)')

highlightTab:AddButton("Copy Invite", function()
	local _clipfunc = setclipboard or toclipboard
	_clipfunc('https://discord.gg/SaGSTVmKM')
end)
